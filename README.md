# Short-link
Short-link is a small URL shortener built for learning system design concepts with a simple Spring Boot backend, an Angular frontend, and Redis for caching and distributed rate-limit state.
...
End-to-End Flow
1. Create short URL
The Angular frontend sends POST /api/shorten
The backend controller extracts the client IP
The rate limiter checks whether that client is allowed
The service either uses the provided custom alias or generates a random Base62 code
The backend stores UrlData in the in-memory map
The backend caches shortCode -> originalUrl in Redis
...
timestamps
2. Open short URL
The browser requests /api/{shortCode}
The backend first checks Redis cache for the original URL
If not found in Redis, it checks the in-memory urlMappings
If the URL is expired, it is marked inactive and the request returns 404
If found, the backend records a click and responds with HTTP 302 Found
The browser follows the redirect to the original URL
3. Load stats
The frontend sends GET /api/stats/{shortCode}
The backend reads UrlData
The backend returns click count, creator IP, active status, and timestamps
4. Scheduled cleanup
Spring scheduling is enabled in the application
CleanupScheduler runs every configured interval
Expired URLs are marked inactive
Related Redis cache entries are deleted
...
Build Redis key from client IP
Read RateLimitData from Redis
If missing, use or create local in-memory state
Check whether the current request is still in the same minute window
Reset the minute counter if the old window expired
...
Short-link.base-url
Short-link.short-code.length
Short-link.short-code.max-attempts
Short-link.cache.ttl-minutes
Short-link.rate-limit.requests-per-minute
Short-link.rate-limit.requests-per-hour
Short-link.cleanup.interval-minutes
